...
from dataset.dataset_finetune_cgcnn import collate_pool, get_train_val_test_loader, get_loader
...
class FineTune(object):
    def __init__(self, config, log_dir):
        ...
        self.train_dataset = CGData(self.config['task'], **self.config['dataset'], split='train')
        self.validate_dataset = CGData(self.config['task'], **self.config['dataset'], split='val')
        self.test_dataset = CGData(self.config['task'], **self.config['dataset'], split='test')
        self.random_seed = self.config['random_seed']
        collate_fn = collate_pool
        self.train_loader = get_loader(self.train_dataset, collate_fn=collate_fn, shuffle=True)
        self.valid_loader = get_loader(self.validate_dataset, collate_fn=collate_fn, shuffle=False)
        self.test_loader = get_loader(self.test_dataset, collate_fn=collate_fn, shuffle=False)
        ...
        if len(self.train_dataset) < 500:
            ...
            sample_data_list = [self.train_dataset[i] for i in range(len(self.datatrain_datasetset))]
        else:
    ...
    def train(self):
        ...
        if self.config['fine_tune_from'] != 'scratch':
