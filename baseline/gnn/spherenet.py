...
def xyz_to_dat(pos, edge_index, num_nodes, use_torsion=False):
    ...
    b = torch.cross(pos_ji, pos_jk, dim=-1).norm(
        dim=-1
    ...
    if use_torsion:
        ...
        plane1 = torch.cross(pos_ji, pos_j0, dim=-1)
        plane2 = torch.cross(pos_ji, pos_jk, dim=-1)
...
class update_e(torch.nn.Module):
    ...
    def forward(self, x, emb, idx_kj, idx_ji):
        rbf0, sbf, t = emb
        x1, _ = x
        rbf0 = rbf0.to(torch.float32)
        sbf = sbf.to(torch.float32)
...
class update_v(torch.nn.Module):
    ...
    def forward(self, e, i):
        _, e2 = e
        v = scatter(e2, i, dim=0, reduce="mean")
        v = self.lin_up(v)
        for lin in self.lins:
            v = self.act(lin(v))
        v = self.lin(v)
...
class update_u(torch.nn.Module):
    ...
    def forward(self, u, v, batch):
        u += scatter(v, batch, dim=0, reduce="mean")
...
class SphereNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.init_e = init(
            num_radial, hidden_channels, act, use_node_features=use_node_features
        )
    ...
    def forward(self, z, pos, batch):
        ...
        v = self.init_v(e, i)
        u = self.init_u(
            torch.zeros_like(scatter(v, batch, dim=0, reduce="mean")), v, batch
        )
