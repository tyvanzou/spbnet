...
class GraphEmbeddings(nn.Module):
    ...
    def __init__(
        ...
        self.charge_embedding = nn.Linear(1, atom_fea_len)
        self.convs = nn.ModuleList(
            [
                ConvLayer(atom_fea_len=atom_fea_len, nbr_fea_len=nbr_fea_len)
                for _ in range(n_conv)
    ...
    def forward(
        ...
        if charge is not None:
            charge.unsqueeze_(-1)
            charge_fea = self.charge_embedding(charge)
            atom_fea += charge_fea
        for conv in self.convs:
            atom_fea = conv(atom_fea, nbr_fea, nbr_idx)  # [N', atom_fea_len]
        atom_fea = self.fc(atom_fea)  # [N', hid_dim]

        new_atom_fea, new_atom_fea_pad_mask, mo_labels = self.reconstruct_batch(
            atom_fea, crystal_atom_idx, moc
        )
    ...
    def reconstruct_batch(self, atom_fea, crystal_atom_idx, moc):
        ...
        return new_atom_fea, new_atom_fea_pad_mask, mo_labels
