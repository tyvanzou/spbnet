...
class Dataset(torch.utils.data.Dataset):
    ...
    # def handle_griddata(self, file_griddata: Path, GRID=30):
    #     griddata = self.load_griddata(file_griddata)  # [30*30*30, 20] uint8
    #     griddata = griddata.astype(np.float32)
    #     griddata = griddata.reshape(GRID, GRID, GRID)

    #     return torch.from_numpy(griddata).float(), None
    ...
    def __getitem__(self, index):
        ...
        if 'agc_label' in self.feats or 'agc_pred' in self.feats:
    ...
    @staticmethod
    def collate(batch, img_size, feats):
        """
        collate batch
        Args:
        ...
        # atom grid
        if 'agc_label' in feats or 'agc_pred' in feats:
