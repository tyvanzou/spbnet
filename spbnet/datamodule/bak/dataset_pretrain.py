...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.config['useMoc']:
            with (data_dir / 'moc.json').open('r') as f:
                self.cifid2moc = json.load(f) # map[str, List[int] (index of metal atoms)]
            # self.
    ...
    def get_coulomb(self, cifid: str):
        file_coulomb = self.data_dir / "coulomb_10" / f"{cifid}.npy"
        coulomb_npy = np.load(file_coulomb)
        mean = coulomb_npy.mean()
        std = coulomb_npy.std()
        # coulomb_npy[coulomb_npy < mean - 5 * std] = mean - 5 * std
        # coulomb_npy[coulomb_npy > mean + 5 * std] = mean + 5 * std
        coulomb_npy /= std
        potential = torch.from_numpy(coulomb_npy).float()
    ...
    @staticmethod
    def collate(batch, img_size, config):
        """
        collate batch
        Args:
        ...
        if config['coulomb']:
            ...
            dict_batch["coulomb"] = batch_coulomb.view(batch_coulomb.shape[0], 10, 10, 10)
        
        # moc
        # automatic collate, get List[List[int]]
