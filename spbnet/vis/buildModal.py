...
def buildModal(cif_path: Path, tgt_dir: Path):
    cifid = cif_path.stem
    ...
    if ifProcessed():
        warn(f"{cifid} have been processed, return")
        return
    for suffix in [
        "graphdata",
        "grid",
...
@click.option("--target-dir", "-T", type=click.Path(file_okay=False, path_type=Path))
def buildModalCli(cif_path, target_dir):
    buildModal(cif_path, target_dir)
